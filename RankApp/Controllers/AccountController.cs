...
using RankApp.Models.AccountViewModels;
using RankApp.Services;
using RankApp.Data;
namespace RankApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(
            ...
            RegisterBufContext Regcontext,
            NavantagennyaContext Navantcontext, 
        ...
        //[HttpGet]
        //[AllowAnonymous]
        //public IActionResult RegFromCode(RegFromCodeViewModel model, string returnUrl = null)
        //{
        //    ViewData["ReturnUrl"] = returnUrl;
        //    if (ModelState.IsValid)
        //    {
        ...
        [HttpGet]
        [AllowAnonymous]
        public IActionResult RegFromCode(string returnUrl = null)
        {
            ViewData["ReturnUrl"] = returnUrl;
            return View();
        }
        ...
        public IActionResult RegFromCode(RegFromCodeViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                var result = dbRegist.RegisterBufs.SingleOrDefault(user => user.RegCode == model.RegCode);
                if (result != null)
                {
                    return View("Register", new RegisterViewModel { RegCode = result.RegCode, PIB = result.PIB });
                }
                else
                {
                    ViewData["Error"] = "Код реєстрації не знайдено";
                    return View(model);
                }
            }
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, PIB = model.PIB };
                var result = await _userManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    var userRoles = await _userManager.GetRolesAsync(user);
                    var allRoles = _roleManager.Roles.ToList();
                    List<string> StartRole = new List<string>();
                    StartRole.Add("User");
                    await _userManager.AddToRolesAsync(user, StartRole);
                    _logger.LogInformation("User created a new account with password.");
